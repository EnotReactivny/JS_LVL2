...
Basket.prototype.collectBasketItems = function(){
	...
	$.get({
		//обращаемся к серверу, воспользуемся интерфейсом сваггер. 

		url: 'https://api.myjson.com/bins/o74am',
		//получаем доступ свойствам класса Baster
		context: this,
		success: function(answer){
			...
			//считаем все quantity в каждом элементе корзины и добавляем в свойства объекта basket
			answer.basket.map(item=>{
				this.count_goods +=item.quantity;
			})
			//считаем все quantity в каждом элементе корзины и добавляем в свойство amount объекта 
			this.amount = answer.full_price;
			...
			//кладем в свйоства корзины текущие товары
			answer.basket.map(item=>{
